...
export class Pilnak extends Component {
    ...
    // filterList2(textToSearch) {
    //     this.setState({
    //         filt_anak1: this.state.anak1.filter(i => i.Sumedang.toLowerCase().includes(textToSearch)),
    //     });
    // }
    render() {
                ...
                {/* <ScrollView horizontal={true}>
                    <View style={{ justifyContent: 'space-between', flexDirection: 'row', textAlign: 'center', alignContent: 'space-between', alignItems: 'center' }}>
                        <TouchableOpacity style={style.pencarian} onPress={() => { this.setState({ sa: true }) }}>
                            <Text style={{ textAlign: 'center', }}>Semua Anak</Text>
                        </TouchableOpacity>

                        <TouchableOpacity style={style.pencarian} onPress={() => { this.setState({ st: true }) }}>
                            <Text style={{ textAlign: 'center', alignSelf: 'center' }}>Jenis</Text>
                        </TouchableOpacity>

                        <TouchableOpacity style={style.pencarian} onPress={() => { this.setState({ jk: true }) }}>
                            <Text style={{ textAlign: 'center', }}>Jenis Kelamin</Text>
                        </TouchableOpacity>
                    </View>
                </ScrollView> */}

                {/* <View style={{ flexDirection: 'row', justifyContent: 'space-around' }}>
                    <Text style={{ fontWeight: 'bold', fontSize: 12, marginLeft: -30, marginTop: 10 }}>Wilayah terdekat</Text>
                    <Text style={{ marginTop: 10, marginRight: -40 }}>Lihat semuanya</Text>
                </View> */}
                <FlatList
                    // horizontal={true}
                    data={filter_sta}
                    renderItem={({ item }) => (
                        <TouchableOpacity onPress={() => { this.props.navigation.navigate('Detail', { id_anak: item.id_anak, item: item }), this.setState({ detail: item }) }}>
                            ...
                            {/* nama: item.full_name, tempat: item.tempat_lahir, tgllahir: item.tanggal_lahir,
                            hobi: item.hobi, anake: item.anak_ke, tiggalbersama: item.tinggal_bersama, 
                            jaram: item.jarak_rumah, transportasi: item.transportasi ,prestasi: item.prestasi */}
