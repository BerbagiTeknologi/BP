...
export class ProfilTutoradmin extends Component {
  constructor(props) {
    super(props)
    this.state = {
      detailtutor: [],
      ...
      pendidikan: this.props.route.params.item.pendidikan,
      mapel: this.props.route.params.item.mapel,
      nama: this.props.route.params.item.nama,
      ...
      detail: this.props.route.params.item.id_users,
      foto: {
        name: '',
        type: '',
  ...
  // GetdetailCabangAPi() {
  //   fetch('https://kilauindonesia.org/datakilau/api/DetailCabang/' + this.props.route.params.item.id_users)
  //     .then(res => {
  ...
  // }
  GetShelterAPi() {
    fetch('https://kilauindonesia.org/datakilau/api/shelter/' + this.state.wilbin)
      .then(res => {
        if (res.status === 200) return res.json();
      })
      .then(resdata => {
  ...
  GetWilbinAPi() {
    fetch('https://kilauindonesia.org/datakilau/api/wilbin/' + this.state.kacab)
      .then(res => {
        if (res.status === 200) return res.json();
      })
      .then(resdata => {
  ...
  sendData1() {
    {
      ...
      fetch('https://kilauindonesia.org/datakilau/api/tutorupd/' + this.props.route.params.item.id_users, {
        method: 'POST',
        headers: {
  ...
  componentDidMount() {
    ...
    this.GetKacabAPi();
    this.GetShelterAPi();
    console.log(this.state.kacab, this.state.wilbin, this.state.shelter);
  }
  render() {
    return (
      <View style={{ flex: 1 }}>
              ...
              <View style={style.form}>
                ...
                <View
                  ...
                  <Picker
                    ...
                    <Picker.Item style={{ fontSize: 12 }} label={this.props.route.params.item.nama_kacab} value={'0'} key={'0'} />
                    {
              ...
              {/* <View style={style.form}>
                ...
                <View style={style.kotakpicker}>
                  <Picker
                    style={style.Textinputcss}
                    selectedValue={this.state.kacab}
                    value={this.props.route.params.item.nama_kacab}
                    onValueChange={(itemValue,) => {
                      this.setState({
                        kacab: itemValue,

                      })
                      console.log(this.state.kacab)
                    }}
                  >
                    <Picker.Item style={{ height: '100%', width: '100%', fontSize: 12, color: '#000' }} label={this.props.route.params.item.nama_kacab.toString()} value={''} key={''} />
                    {
                      this.state.getkacab.map((cab) =>
                        <Picker.Item style={{ height: '100%', width: '100%', fontSize: 12, color: '#000' }} label={cab.nama_kacab.toString()} value={cab.id_kacab} key={cab.id_kacab} />
                      )}
              ...
              <View style={style.form}>
                ...
                <View
                  ...
                  <Picker
                    ...
                    onFocus={() => { this.GetKacabAPi() }}
                    onValueChange={(itemValue, getkacab) => {
                      this.setState({
                        wilbin: itemValue
                      })
                    }}>
                    {/* <Picker.Item style={{ fontSize: 12 }} label={detail.id_prov === null ? 'Pilih Provinsi' : detail.nama_prov} value={'0'} key={'0'} /> */}
                    <Picker.Item style={{ fontSize: 12 }} label={this.props.route.params.item.nama_wilbin} value={'0'} key={'0'} />
                    {
                      this.state.getwilbin.map((bin) =>
                        <Picker.Item style={{ height: '100%', width: '100%', fontSize: 12, color: '#000' }} label={bin.nama_wilbin.toString()} value={bin.id_wilbin} key={bin.id_wilbin} />
                      )}
                  </Picker>
                </View>
              </View>
              ...
              {/* <View style={style.form}>
                ...
                <View style={style.kotakpicker}>
                  <Picker
                    style={style.Textinputcss}
                    selectedValue={this.state.wilbin}
                    value={this.props.route.params.item.nama_wilbin}
                    onFocus={() => { this.GetKacabAPi() }}
                    onValueChange={(itemValue, getkacab) => {
                      this.setState({
                        wilbin: itemValue
              ...
              <View style={style.form}>
                ...
                <View style={style.kotakpicker}>
                  <Picker
                    ...
                    value={this.props.route.params.item.nama_shelter}
                    onFocus={() => { this.GetWilbinAPi() }}
                    onValueChange={(itemValue, getwilbin) =>
                      this.setState({ shelter: itemValue, show: 1 })
                    }>
                    <Picker.Item style={{ fontSize: 12 }} label={this.props.route.params.item.nama_shelter} value={'0'} key={'0'} />
                    {
                      this.state.getshelter.map((shel) =>
                        <Picker.Item style={{ height: '100%', width: '100%', fontSize: 12, color: '#000' }} label={shel.nama_shelter.toString()} value={shel.id_shelter} key={shel.id_shelter} />
                      )}
...
export default ProfilTutoradmin
